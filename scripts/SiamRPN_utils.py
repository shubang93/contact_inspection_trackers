...
def im_to_numpy(img):
    img = to_numpy(img)
    print("im_to_numpy", img.shape)
    img = np.transpose(img, (1, 2, 0))  # H*W*C
    return img


def im_to_torch(img):
    print("img_to_torch", img.shape)
...
def get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans, out_mode='torch', new=False):
    ...
    if any([top_pad, bottom_pad, left_pad, right_pad]):
        ...
        for i in range(k):
            te_im[top_pad:top_pad + r, left_pad:left_pad + c,i] = im
        if top_pad:
        ...
        im_patch_original = np.zeros((im_patch_original.shape[0], im_patch_original.shape[1], 3))
        for i in range(k):
            im_patch_original[:,:,i] = im[int(context_ymin):int(context_ymax + 1), int(context_xmin):int(context_xmax + 1)]
    print('Sape', im_patch_original.shape, model_sz)

    if not np.array_equal(model_sz, original_sz):
        im_patch = cv2.resize(im_patch_original, (model_sz, model_sz))  # zzp: use cv to get a better speed
        print("if", im_patch.shape)
    else:
